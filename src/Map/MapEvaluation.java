...
public class MapEvaluation {
    ...
    double[] characteristics = new double [9];		
     ...
     */
    for(int i = 1; i < lmap.length-1; i++)for(int j = 1; j < lmap[0].length-1; j++)if(lmap[i][j]==2){
        dx = i;
        dy = j;
    ...
    for(int i = 1; i < lmap.length-1; i++)for(int j = 1; j < lmap[0].length-1; j++)if(lmap[i][j]==3){
        sx = i;
        sy = j;
        ...
        if(lmap[i][j-1]==1) {
        	spawnsurround++;}
        if(spawnsurround<minspawnsurround)minspawnsurround=spawnsurround;
        spawnsurround=0;
